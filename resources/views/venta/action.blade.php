                ...
                <div class="row">
                        ...
                        <div class="p-3 border border-3 border-primary">
                            <div class="row">
                                <div class="col-md-12 mb-2">
                                    <select name="iDetProductoID" id="iDetProductoID" class="form-control selectpicker" data-live-search="true">
                                        <option value="" data-precio="0" disabled selected>Seleccione un producto</option>
                                        @foreach($productos as $pro)
                                        <option value="{{$pro->iProID}}" data-precio="{{$pro->fProPrecioVenta}}">{{$pro->cProNombre}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 mb-2">
                                    <label for="iDetCantidad" class="form-label">Cantidad</label>
                                    <input type="number" name="iDetCantidad" id="iDetCantidad" class="form-control" value="">
                                </div>
                                <div class="col-md-6 mb-2">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="p-3 border border-3 border-success">
                            <div class="row">
                                <div class="col-md-12 mb-2">
                                    <label for="iVenClienteID" class="form-label">Cliente</label>
                                    <select name="iVenClienteID" id="iVenClienteID" class="form-control">
                                        @foreach($clientes as $cliente)
                                        <option value="{{ $cliente->iCliID }}">
                                            {{ $cliente->persona->cPerDNI }}-{{ $cliente->persona->cPerApellido }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 mb-2">
                                    <label for="fVenTotal" class="form-label">Total Compra</label>
                                    <input readonly type="text" name="fVenTotal" id="fVenTotal" class="form-control border-success" value="">
                                </div>
                                <div class="col-md-6 mb-2">
                                    <label for="dVenFecha" class="form-label">Fecha</label>
                                    <input readonly type="date" name="dVenFecha" id="dVenFecha" class="form-control border-success" value="<?php echo date('Y-m-d') ?>">
                                </div>
...
<script>
    document.addEventListener('shown.bs.modal', function(event) {
        ...
        const productoSelect = modal.querySelector('#iDetProductoID');
    ...
    function agregarProducto() {
        let idProducto = $('#iDetProductoID').val();
        let nombrePorducto = $('#iDetProductoID option:selected').text();
        let cantidad = $('#iDetCantidad').val();
    ...
    function eliminarProducto(indice) {
        ...
        if ($('#tablaDetalle tbody tr').length === 0) {
            suma = 0;
            $('#fVenTotal').val(round(suma).toFixed(2));
            desactivarBotones();
        } else {
            $('#fVenTotal').val(round(suma).toFixed(2));
        }
